 ...
 *
 */
public class SelectSemanticAnalyzerTest
{
        ...
        throws Exception
    {
        String sql =
            "select S1.id as aid,S2.name,count(1) as c " + "from S1(id > 1)[range unbounded]  "
                ...
                + "where s1.id=1 group by aid having c>10 order by aid asc limit 10";
        ...
        //From clause
        assertFromClause(analyzeConext.getFromClauseContext());
        //select clause
        assertSelectClause(analyzeConext.getSelectClauseContext());
        //where clause
        assertWhereClause(analyzeConext.getWhereClauseContext());
        //groupby clause
        assertGroupbyClause(analyzeConext.getGroupbyClauseContext());
        //having clause
        assertHavingClause(analyzeConext.getHavingClauseContext());
        //orderby clause
        assertSortbyClause(analyzeConext.getOrderbyClauseContext());
        //limit clause
        assertLimitClause(analyzeConext.getLimitClauseContext());
    }
    ...
    private void assertHavingClause(FilterClauseAnalzyeContext parseContext)
    {
        ...
        assertTrue(exp3.getConstValue().equals(ConstInTestCase.I_10));
    }
    ...
    private void assertWhereClause(FilterClauseAnalzyeContext whereClauseContext)
    {
        assertTrue(whereClauseContext.getExpdes().size() == ConstInTestCase.I_1);
        assertTrue(whereClauseContext.getExpdes().get(0) instanceof BinaryExpressionDesc);
        
        BinaryExpressionDesc exp1 = (BinaryExpressionDesc)whereClauseContext.getExpdes().get(ConstInTestCase.I_0);
    ...
    private void assertSelectClause(SelectClauseAnalyzeContext parseContext)
    {
        ...
        //TODO outputschema的列类型目前还没有确定
        //        assertTrue(parseContext.getOutputSchema().getCols().size() == 3);
        //        assertTrue(parseContext.getOutputSchema().getCols().get(0).getType().equals(String.class.getName()));
        //        assertTrue(parseContext.getOutputSchema().getCols().get(1).getType().equals(String.class.getName()));
        //        assertTrue(parseContext.getOutputSchema().getCols().get(2).getType().equals(String.class.getName()));
        ...
        throws SemanticAnalyzerException
    {
        ...
        ConstExpressionDesc exp3 = (ConstExpressionDesc)exp1.getArgExpressions().get(ConstInTestCase.I_1);
        assertTrue(exp2.getProperty().equals("id"));
        assertTrue(exp3.getConstValue().equals(ConstInTestCase.I_1));
        ...
        assertTrue(exp23.getConstValue().equals(ConstInTestCase.I_1));
        ...
        assertTrue(exp53.getConstValue().equals(ConstInTestCase.I_1));
    }
